...
{
    variables = {};
    // Step 1: Generate a variables lookup table
    var cell = 0;
    ...
    for (var i=0;i<raw.length;i++)
    {
        ...
        switch (token.length)
        {
            case 2:
                if (token[1] == "DAT")
                {
                    variables[token[0]] = cell;
                    document.getElementById("cell_"+pad(cell)).value=0;
                }
                break;
            ...
            case 3:
                ...
                {
                    variables[token[0]] = cell;
                    document.getElementById("cell_"+pad(cell)).value = token[2];
                }
                break;
        }
        if (!(token[0] in table))
    ...
    for (var i=0;i<raw.length;i++)
    {
        ...
        switch (token.length)
        {
            case 2:
                if (token[0] in table)
                {
                    opcode = token[0]; operand = token[1];
                    if (operand in variables)
                    {
                        operand = variables(operand);
                    }

                    var denary = table[opcode] + operand;
                    document.getElementById("cell_"+pad(cell)).value = denary;
                    cell = cell + 1;
